...
def run_calculate_aq(config: DictConfig) -> None:
    ...
    enhanced_folder = Path(config.path.enhanced_folder)
    ...
    for idx, scene_listener_pair in enumerate(scene_listener_pairs, 1):
        ...
        logger.info(f"Song score: {song_score}")
        ...
        overall_scores = np.append(overall_scores, song_score)

        if idx % 10 == 0 and idx != 0:
            logger.info(f"Overall score: {np.mean(overall_scores).round(3)}")

    logger.info(f"Final Overall score: {np.mean(overall_scores).round(3)}")
