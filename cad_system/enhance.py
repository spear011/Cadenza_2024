...
def enhance(config: DictConfig) -> None:
    ...
    enhanced_folder = Path(f"enhanced_signals_{dates}")
    ...
    if config.separator.model == "demucs":
        ...
        if config.separator.ckpt:
            try:
                from collections import OrderedDict
                state_dict = torch.load(config.separator.ckpt, map_location=device)
                new_state_dict = OrderedDict()
                for k, v in state_dict.items():
                    name = k[7:] # remove `module.`  ## module 키 제거
                    new_state_dict[name] = v
                separation_model.load_state_dict(new_state_dict)
                print(f"Loaded model from {config.separator.ckpt}")
            except:
                separation_model.load_state_dict(torch.load(config.separator.ckpt, map_location=device))
                print(f"Loaded model from {config.separator.ckpt}")
