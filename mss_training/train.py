...
import warnings
import wandb
wandb.init(project='cadenza', 
           group='demucs',
           name='hdemucs',
           notes='finetuning w/aug w/official data',
           entity='cadenza')
...
def valid(model, args, config, device, verbose=False):
    ...
    for path in all_mixtures_path:
        ...
        if args.model_type == 'htdemucs' or args.model_type == 'hdemucs' or args.model_type == 'bs_hdemucs':
            mixture = mix.T
...
def train_model(args):
    ...
    print('Train samples: {}'.format(len(trainset)))
    ...
    if args.start_check_point != '':
        ...
        elif args.model_type == 'dtt_net' and args.our_model == "":
    ...
    for epoch in range(config.training.num_epochs):
        ...
        for i, (batch, mixes) in enumerate(pbar):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                ...
                elif args.model_type == 'htdemucs'  or args.model_type == 'bs_hdemucs' or args.model_type == 'hdemucs':
                    dims = tuple(range(2, y.dim()))
                    y_ = model(x)
                    loss = F.l1_loss(y_, y, reduction='none')
        ...
        if epoch > 0:
            ...
            wandb.log({"valid sdr":sdr_avg, "Training loss":loss_val / total}, step=epoch)
            if sdr_avg > best_sdr and sdr_avg > 0:
                store_path = args.results_path + '/ckpt/' + '/model_{}_ep_{}_sdr_{:.4f}.ckpt'.format(args.model_type, epoch, sdr_avg)
            ...
            # Save last
            store_path = args.results_path + '/ckpt/' + '/last_{}.ckpt'.format(args.model_type)
            state_dict = model.state_dict() if type(device_ids) == int else model.module.state_dict()
            torch.save(
                state_dict,
                store_path
