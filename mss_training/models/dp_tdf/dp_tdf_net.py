...
from .spectro import spectro, ispectro
...
class DPTDFNet(AbstractModel):
    ...
    def forward(self, x):
        ...
        for i in range(self.n):
            x = self.us[i](x)
            # print(f"us{i} in: {x.size(2)}")
            # print(f"ds{i} out: {ds_outputs[-i - 1].size(2)}")
            x = nn.functional.pad(x, (0, 0, 0, ds_outputs[-i - 1].size(2) - x.size(2)), "constant", 0)
            x = x * ds_outputs[-i - 1]
        ...
        if x.shape[-1] != length:
            x = nn.functional.pad(x, (0, length - x.shape[-1]), "reflect", 0)
